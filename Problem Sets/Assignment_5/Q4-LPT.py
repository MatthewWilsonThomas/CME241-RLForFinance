...
if __name__ == '__main__':
    ...
    print("Reach price_diff")

    price_diff = [lambda p_s: beta * p_s.shares + theta for _ in range(num_time_steps)]
    # Alter the dynamics to P_{t+1} = P_t * e^{Z_t}l Z_t ~ N(mu_z, sigma_z)
    dynamics = [lambda p_s: SampledDistribution(
        sampler = lambda p_s=p_s: p_s.prices * np.exp(np.random.normal(loc=mu_z, scale=sigma_z)),
